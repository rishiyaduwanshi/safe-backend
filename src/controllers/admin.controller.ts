import { Request, Response, NextFunction } from 'express';
import mongoose from 'mongoose';
import ModeratorModel from '@/models/moderator.model';
import UserModel from '@/models/user.model';
import { ReportModel } from '@/models/report.model';
import Profile from '@/models/profile.model';
import { CssEventModel } from '@/models/cssEvent.model';
import appResponse from '@/utils/appResponse';
import { BadRequestError, NotFoundError } from '@/utils/appError';
import { HttpStatus, UserRole } from '@/types/common.types';
import {
  CreateModeratorInput,
  ListUsersQueryInput,
  UpdateModeratorPermissionsInput,
} from '@/validations/validate.admin';

// ─── Create Moderator ────────────────────────────────────────────────────────

export const createModerator = async (
  req: Request<unknown, unknown, CreateModeratorInput>,
  res: Response,
  next: NextFunction
): Promise<void> => {
  try {
    const { name, email, password, permissions } = req.body;

    const existing = await ModeratorModel.findOne({ email });
    if (existing) {
      throw new BadRequestError('A moderator with this email already exists');
    }

    const moderator = await ModeratorModel.create({
      name,
      email,
      password,       // pre-save hook hashes it
      permissions,
      createdBy: new mongoose.Types.ObjectId(req.user!.id),
    });

    appResponse(res, {
      statusCode: HttpStatus.CREATED,
      message: 'Moderator created successfully',
      data: {
        id: moderator._id,
        name: moderator.name,
        email: moderator.email,
        permissions: moderator.permissions,
        isActive: moderator.isActive,
      },
    });
  } catch (error) {
    next(error);
  }
};

// ─── List All Moderators ─────────────────────────────────────────────────────

export const listModerators = async (
  _req: Request,
  res: Response,
  next: NextFunction
): Promise<void> => {
  try {
    const moderators = await ModeratorModel.find()
      .select('-__v')
      .populate('createdBy', 'name email');

    appResponse(res, {
      message: 'Moderators fetched successfully',
      data: moderators,
    });
  } catch (error) {
    next(error);
  }
};

// ─── Get Single Moderator ────────────────────────────────────────────────────

export const getModerator = async (
  req: Request<{ id: string }>,
  res: Response,
  next: NextFunction
): Promise<void> => {
  try {
    const moderator = await ModeratorModel.findById(req.params.id)
      .select('-__v')
      .populate('createdBy', 'name email');

    if (!moderator) throw new NotFoundError('Moderator not found');

    appResponse(res, {
      message: 'Moderator fetched successfully',
      data: moderator,
    });
  } catch (error) {
    next(error);
  }
};

// ─── Update Permissions ───────────────────────────────────────────────────────

export const updateModeratorPermissions = async (
  req: Request<{ id: string }, unknown, UpdateModeratorPermissionsInput>,
  res: Response,
  next: NextFunction
): Promise<void> => {
  try {
    const moderator = await ModeratorModel.findByIdAndUpdate(
      req.params.id,
      { permissions: req.body.permissions },
      { new: true }
    );

    if (!moderator) throw new NotFoundError('Moderator not found');

    appResponse(res, {
      message: 'Permissions updated successfully',
      data: {
        id: moderator._id,
        permissions: moderator.permissions,
      },
    });
  } catch (error) {
    next(error);
  }
};

// ─── Toggle Active Status ─────────────────────────────────────────────────────

export const toggleModeratorStatus = async (
  req: Request<{ id: string }>,
  res: Response,
  next: NextFunction
): Promise<void> => {
  try {
    const moderator = await ModeratorModel.findById(req.params.id);
    if (!moderator) throw new NotFoundError('Moderator not found');

    moderator.isActive = !moderator.isActive;
    await moderator.save();

    appResponse(res, {
      message: `Moderator ${moderator.isActive ? 'activated' : 'deactivated'} successfully`,
      data: { id: moderator._id, isActive: moderator.isActive },
    });
  } catch (error) {
    next(error);
  }
};

// ─── Delete Moderator ─────────────────────────────────────────────────────────

export const deleteModerator = async (
  req: Request<{ id: string }>,
  res: Response,
  next: NextFunction
): Promise<void> => {
  try {
    const moderator = await ModeratorModel.findByIdAndDelete(req.params.id);
    if (!moderator) throw new NotFoundError('Moderator not found');

    appResponse(res, {
      statusCode: HttpStatus.NO_CONTENT,
      message: 'Moderator deleted successfully',
    });
  } catch (error) {
    next(error);
  }
};

// ─── Citizen Users Management (Admin) ──────────────────────────────────────

export const listUsers = async (
  req: Request<unknown, unknown, unknown, ListUsersQueryInput>,
  res: Response,
  next: NextFunction
): Promise<void> => {
  try {
    const { q, status, page, limit } = req.query;

    const filter: Record<string, unknown> = { role: UserRole.USER };

    if (status === 'active') filter.isActive = { $ne: false };
    if (status === 'inactive') filter.isActive = false;

    if (q) {
      filter.$or = [
        { name: { $regex: q, $options: 'i' } },
        { email: { $regex: q, $options: 'i' } },
      ];
    }

    const skip = (page - 1) * limit;

    const [rawItems, total] = await Promise.all([
      UserModel.find(filter)
        .sort({ createdAt: -1 })
        .skip(skip)
        .limit(limit)
        .select('name email role isActive profileId css cssInitialized createdAt updatedAt')
        .lean(),
      UserModel.countDocuments(filter),
    ]);

    const items = rawItems.map((u) => ({
      ...u,
      isActive: u.isActive !== false,
    }));

    appResponse(res, {
      message: 'Users fetched successfully',
      data: {
        items,
        page,
        limit,
        total,
      },
    });
  } catch (error) {
    next(error);
  }
};

export const getUser = async (
  req: Request<{ id: string }>,
  res: Response,
  next: NextFunction
): Promise<void> => {
  try {
    const user = await UserModel.findOne({ _id: req.params.id, role: UserRole.USER })
      .select('name email role isActive profileId css cssInitialized createdAt updatedAt')
      .lean();

    if (!user) throw new NotFoundError('User not found');

    const counts = await ReportModel.aggregate([
      { $match: { submittedBy: new mongoose.Types.ObjectId(req.params.id) } },
      { $group: { _id: '$status', count: { $sum: 1 } } },
    ]);

    const stats = {
      totalReports: counts.reduce((acc, cur) => acc + (cur?.count ?? 0), 0),
      byStatus: counts.reduce<Record<string, number>>((acc, cur) => {
        if (cur?._id) acc[String(cur._id)] = cur.count;
        return acc;
      }, {}),
    };

    const profile = user.profileId
      ? await Profile.findById(user.profileId)
        .select('name licenseNumber driverId phone email address vehicleType issueDate expiryDate status state city pincode createdAt updatedAt')
        .lean()
      : null;

    const reports = await ReportModel.find({ submittedBy: new mongoose.Types.ObjectId(req.params.id) })
      .sort({ createdAt: -1 })
      .limit(50)
      .select('reportText category severity confidence status needsReview rejectionReason location createdAt updatedAt')
      .lean();

    const cssHistory = await CssEventModel.find({ user: new mongoose.Types.ObjectId(req.params.id) })
      .sort({ createdAt: -1 })
      .limit(50)
      .select('decision categoryId delta previousCss nextCss report moderator createdAt')
      .populate('moderator', 'name email')
      .lean();

    appResponse(res, {
      message: 'User fetched successfully',
      data: {
        user: { ...user, isActive: user.isActive !== false },
        profile,
        stats,
        reports,
        cssHistory,
      },
    });
  } catch (error) {
    next(error);
  }
};

export const toggleUserStatus = async (
  req: Request<{ id: string }>,
  res: Response,
  next: NextFunction
): Promise<void> => {
  try {
    const user = await UserModel.findOne({ _id: req.params.id, role: UserRole.USER });
    if (!user) throw new NotFoundError('User not found');

    user.isActive = user.isActive === false;
    await user.save();

    appResponse(res, {
      message: `User ${user.isActive ? 'activated' : 'deactivated'} successfully`,
      data: { id: user._id, isActive: user.isActive },
    });
  } catch (error) {
    next(error);
  }
};
